...
require_once("include/instock_variables.php");
...
$layout->version = 3;
	$layout->bootstrapTheme = "cerulean";
		$layout->customCssPageName = "instock_detailspreview";
...
$page_layouts["instock_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["VIN"]));
		$keylink.="&key2=".runner_htmlspecialchars(rawurlencode(@$data["Location_ID"]));
	...
	//	Avalible - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("Avalible", $data, $keylink);
			$row["Avalible_value"] = $value;
			$format = $pSet->getViewFormat("Avalible");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("Avalible")))
				$class = ' rnr-field-number';
			$row["Avalible_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("instock", "detailspreview"));
