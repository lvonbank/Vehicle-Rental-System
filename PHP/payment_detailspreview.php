...
$layout->version = 3;
	$layout->bootstrapTheme = "cerulean";
		$layout->customCssPageName = "payment_detailspreview";
...
$page_layouts["payment_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["Payment_ID"]));
	
	
	//	Payment_ID - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("Payment_ID", $data, $keylink);
			$row["Payment_ID_value"] = $value;
			$format = $pSet->getViewFormat("Payment_ID");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("Payment_ID")))
				$class = ' rnr-field-number';
			$row["Payment_ID_class"] = $class;
	//	PaymentType - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("PaymentType", $data, $keylink);
			$row["PaymentType_value"] = $value;
			$format = $pSet->getViewFormat("PaymentType");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("PaymentType")))
				$class = ' rnr-field-number';
			$row["PaymentType_class"] = $class;
	//	TransactionNum - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("TransactionNum", $data, $keylink);
			$row["TransactionNum_value"] = $value;
			$format = $pSet->getViewFormat("TransactionNum");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("TransactionNum")))
				$class = ' rnr-field-number';
			$row["TransactionNum_class"] = $class;
	//	Cust_ID - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("payment", "detailspreview"));
