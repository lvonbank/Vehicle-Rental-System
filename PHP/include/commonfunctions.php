...
function checkTableName($shortTName, $type=false)
{
	...
	if ("address" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("billing" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("booking" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("customer" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("discount" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("instock" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("location" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("payment" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("vehicle" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("revenue_by_location" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	return false;
}
...
function AfterFBLogIn( $pUsername, $pPassword, $pDisplayUsername, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("login")
